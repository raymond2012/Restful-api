...
class test():
    ...
    def test_login_and_logout(self):
        ...
        assert self.login.status_code == 200, "Expected Status code is 200 but the status code is " + str(self.login.status_code)
        # Check the token and user_id are gotten from the login response successfully
        assert self.login_result['token'] is not None and self.login_result['user_id'] > 0, "Expected token and user_id exist but they are None"
        # Logout the user
        ...
        assert result_logout.status_code == 200, "Expected Status code is 200 but the status code is " + str(self.result_logout.status_code)
    ...
    def test_get_snap_checking_order_by_creation(self):
        ...
        assert result_full['response'].status_code == 200, "Expected Status code is 200 but the status code is " + str(self.login.status_code)
    ...
    def test_create_snap(self):
        ...
        assert int(snap_id_testing) in result_get_user_snap['list'], "The expected created snap id" + snap_id_testing + " is not in the list from get_snap of a user" + result_get_user_snap['list']
    ...
    def test_get_products_of_a_snap(self):
        ...
        assert all(i == int(snap_id) for i in result_get_products['list']), "An snap_id of the item in the product of a snap result list" + result_get_products['list'] + " is not " + snap_id
    ...
    def test_search_snap_checking_order_by_creation(self):
        ...
        query_second = {"q": "A", "filter": "", "offset": "", "offset_id": "", "limit": "10", "order": "DESC", "orderby": "creation"}
        query_third = {"q": "A", "filter": "", "offset": "", "offset_id": "", "limit": "10", "order": "DESC", "orderby": "creation"}
        query_last = {"q": "A", "filter": "", "offset": "", "offset_id": "", "limit": "10", "order": "DESC", "orderby": "creation"}
        # Get 40 snaps and check the search request successfully
        result_full = self.user.search_snaps(query_full)
        assert result_full['response'].status_code == 200, "Expected Status code is 200 but the status code is " + str(self.result_full['response'].status_code)
        # The offset_id will be defined with the 0th, 10th, 20th and 30th items from the above query
        result_full_list = self.user.search_snaps(query_full)['list']
        query_second['offset_id'] = result_full_list[int(len(result_full_list) / 4 - 1)]
        query_third['offset_id'] = result_full_list[int(len(result_full_list) / 2 - 1)]
        query_last['offset_id'] = result_full_list[int(len(result_full_list) * 3 / 4 - 1)]
        # Combine the 4 queries result in one list
        result_list_combine = self.user.search_snaps(query_first)['list'] + self.user.search_snaps(query_second)['list'] + self.user.search_snaps(query_third)['list'] + self.user.search_snaps(query_last)['list']
        # Compare the results from two types of query are the same
        assert result_full_list == result_list_combine, "The results are not the same. Full Result with limit 40: " + result_full_list + ". Full Result of 4 queries with limit 10: " + result_list_combine
    ...
    def test_get_comment_of_a_snap_checking(self):
        ...
        assert result_get_comment['response'].status_code == 200, "Expected Status code is 200 but the status code is " + str(self.result_get_comment.status_code)
    ...
    def test_get_snap_info_after_login(self):
        home_query = {'snap_id': '7806'}
        self.user.get_snap_info_after_login(home_query)
    ...
    def test_get_user_profile(self):
        result_get_profile = self.user.get_user()
        ...
        assert json.loads(result_get_profile.content.decode('utf-8'))['user_id'] == self.login_result['user_id'], "The user id do not match"
    ...
    def test_user_profile(self):
        ...
        assert result_prof_update.status_code == 200, "Expected Status code: 200 but the status code: " + str(result_prof_update.status_code)
        # Update the user profile picture
        result_prof_pir_update = self.user.upload_user_profile_pic('Testing', encoded_image)
        assert result_prof_pir_update.status_code == 200, "Expected Status code: 200 but the status code: " + str(result_prof_pir_update.status_code)
        # Get the user profile to check the update successfully or not
        result_prof_get = self.user.get_user()
        assert result_prof_get.status_code == 200, "Expected Status code: 200 but the status code: " + str(result_prof_get.status_code)
        ...
        assert result_prof_pir_remove.status_code == 204, "Expected Status code: 204 but the status code: " + str(result_prof_pir_remove.status_code)
        # Get the user profile again to check the profile deleted or not
        assert json.loads(self.user.get_user().content.decode('utf-8'))['user_propic'] is None, "The profile pic is not removed"
    ...
    def test_count_user_follower_and_following(self):
        ...
        assert result_count.status_code == 200, "Expected Status code: 200 but the status code: " + str(result_count.status_code)
        # Get follower and following count from user profile
        result_count_json = json.loads(result_count.content.decode('utf-8'))
        result_profile = self.user.get_user()
        assert result_profile.status_code == 200, "Expected Status code: 200 but the status code: " + str(result_profile.status_code)
    ...
    def test_follow_a_user(self):
        ...
        assert str(json.loads(self.user2.get_following().content.decode('utf-8'))['following'][0]['user_id']) == self.user.get_user_id()
        # User get the follower result and check User2 follow User successfully by user_id
        assert str(json.loads(self.user.get_follower().content.decode('utf-8'))['follower'][0]['user_id']) == self.user2.get_user_id()
        result_user1_follower = int(json.loads(self.user.count_user_follower_and_following().content.decode('utf-8'))['n_follower'])
        assert result_user1_follower > 0
        result_unfollow = self.user2.unfollow_user(self.user.get_user_id())
        assert result_unfollow.status_code == 204, "Expected Status code: 204 but the status code: " + str(result_unfollow.status_code)
        result_get_follower = json.loads(self.user.get_follower().content.decode('utf-8'))['n_follower']
        assert int(result_get_follower) == result_user1_follower - 1, "The Expected follower should be " + str(result_user1_follower - 1) + " but the result is " + str(result_get_follower)
    ...
    def test_favourite_snap(self):
        ...
        assert result_add_fav_snap.status_code == 201, "Expected Status code: 201 but the status code: " + str(result_add_fav_snap.status_code)
        # Get the favourite snap to check the update request sent successfully
        result_get_fav_snap = self.user.get_favourite_snaps()
        assert result_get_fav_snap['response'].status_code == 200, "Expected Status code: 200 but the status code: " + str(result_get_fav_snap.status_code)
        ...
        assert result_remove_fav_snap.status_code == 204, "Expected Status code: 204 but the status code: " + str(result_remove_fav_snap.status_code)
    ...
    def test_favourite_product(self):
        ...
        assert result_add_fav_prod.status_code == 201, "Expected Status code: 201 but the status code: " + str(result_add_fav_prod.status_code)
        # Get the favourite product to check the update successfully or not
        result_get_fav_prod = self.user.get_favourite_products()
        assert result_get_fav_prod['response'].status_code == 200, "Expected Status code: 200 but the status code: " + str(result_get_fav_prod.status_code)
        assert int(snap_product_id) in result_get_fav_prod['list'], "The snap product is added to favourite unsuccessfully"
        # Delete the favourite snap
        ...
        assert int(snap_product_id) not in result_get_fav_prod_again['list'], "The snap product is added to favourite unsuccessfully"
    ...
    def test_get_user_snap_of_a_user(self):
        ...
        assert result_get_user_snap['response'].status_code == 200, "Expected Status code: 200 but the status code: " + str(result_get_user_snap['response'].status_code)
        # Check the result
        assert all(i == int(user_id) for i in result_get_user_snap['list']), "An snap_id of the item in the product of a snap result list" + result_get_products['list'] + " is not " + snap_id

        # assert int(user_id) in result_get_user_snap["list"], "An snap_id of the item in the snap of user result list" + str(result_get_user_snap['list']) + " is not " + user_id
    ...
    def test_search_user(self):
        ...
        assert search_word in result_search['list'], "The search word (" + search_word + ") did not match with the username list " + str(result_search['list'])
    ...
    def test_get_number_of_likes_of_a_user(self):
        ...
        assert json.loads(result_get.content.decode('utf-8'))['likes'] is not None, "Cannot get the likes result"
    ...
    def test_report_user(self):
        ...
        assert result_report.status_code == 201, "Expected Status code: 201 but the status code: " + str(result_report.status_code)

    def test_check_username_valid(self):
        # Check username owned by myself
        result_self = self.user.check_user_valid(json.loads(self.user.get_user().content.decode('utf-8'))['username'])
        result_self_json = json.loads(result_self.content.decode('utf-8'))
        # Check the check request sent successfully
        assert result_self.status_code == 200, "Expected Status code: 200 but the status code: " + str(result_self.status_code)
        # Check the username is valid
        assert result_self_json['result'] is True, "Expected result is true but the result is " + str(result_self_json['result'])
    ...
    def test_social_media_list(self):
        ...
        assert len(result_media_json['platforms']) == 6, "Expected social media item number is 6 but the number is " + str(len(result_media_json['platforms']))

# test_follow_a_user()
# test_get_snap_order()
# test().test_search_snap_checking_order_by_creation()
test().test_get_user_snap_of_a_user()
